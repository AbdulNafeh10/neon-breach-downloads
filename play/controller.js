...
export function padActions(sample,pressed,sprint=false){return {forward:-sample.move.y,strafe:sample.move.x,lookX:sample.look.x,lookY:sample.look.y,aim:sample.buttons[6],fire:sample.buttons[7],firePress:pressed[7],jump:pressed[0],slide:pressed[1]||pressed[11],reload:pressed[2],next:pressed[3],dash:pressed[4],boost:pressed[5],sprint,ping:pressed[12],regroup:pressed[15],ack:pressed[14],cancelPing:pressed[13],menu:pressed[9],accept:pressed[0],back:pressed[1]};}
export class ControllerInput{
 ...
 poll(deadzone=.14){let pads=[];this.error='';try{pads=Array.from(this.read()||[]).filter(p=>p?.connected!==false&&p);}catch{this.error='Controller access is blocked here. Open the game in desktop Chrome or Edge.';}
 ...
 if(this.armed&&pressed[10])this.sprinting=!this.sprinting;
 if(Math.hypot(sample.move.x,sample.move.y)<.1)this.sprinting=false;
 this.previous=sample;const actions=padActions(sample,pressed,this.sprinting);if(!this.armed){actions.fire=false;actions.aim=false;}
