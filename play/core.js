...
export function validState(p){return p&&['x','y','z','yaw','pitch'].every(k=>Number.isFinite(p[k]))&&Math.abs(p.x)<26&&Math.abs(p.z)<21&&p.y>=-.1&&p.y<12&&Math.abs(p.pitch)<2&&Number.isInteger(p.gun)&&p.gun>=0&&p.gun<3;}
