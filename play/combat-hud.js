...
export function mountCombatHUD(doc){
 const root=doc.createElement('div');root.id='combat-assist';root.innerHTML='<div class="incoming-bearing" hidden aria-label="Direction of incoming damage"><i></i></div><div class="reload-meter" hidden><span></span><div><i></i></div></div><section class="death-recap" hidden><small>ELIMINATED BY</small><strong></strong><p></p><span></span></section>';doc.body.append(root);
 ...
 update({player,weapon,mode,paused,now}){
  ...
  bearing.hidden=!last||now-at>1.1||player.dead>0;if(!bearing.hidden){bearing.style.transform='translate(-50%,-50%) rotate('+incomingBearing(player,last.source)+'rad)';bearing.style.opacity=Math.min(1,(1.1-(now-at))*2);}
