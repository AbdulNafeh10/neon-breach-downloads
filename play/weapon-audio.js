...
export function weaponSound(context,noise,destination,index,volume=1,pan=0){
 ...
 if(index===1){burst(750,.025,.4,.2,'bandpass');tone(145,38,0,.28,.5);burst(5800,0,.065,.55,'highpass');burst(1700,.005,.34,.65);burst(3200,.28,.065,.25,'bandpass');tone(420,180,.29,.075,.08,'triangle');burst(2300,.42,.05,.2,'bandpass');}
 else if(index===2){burst(950,.035,.38,.16,'bandpass');burst(3400,.38,.035,.12,'bandpass');tone(240,43,0,.3,.38);burst(7200,0,.045,.55,'highpass');burst(2200,.01,.29,.4);tone(750,180,.3,.06,.06,'triangle');}
 else if(index===3){burst(1700,.018,.045,.15,'bandpass');tone(240,95,0,.055,.2,'triangle');burst(6300,0,.04,.31,'highpass');}
 else if(index===4){burst(2800,.07,.032,.16,'bandpass');tone(195,45,0,.18,.4);burst(4200,0,.055,.45,'highpass');burst(1300,.02,.16,.24);}
 else{burst(2400,.04,.035,.13,'bandpass');tone(170,58,0,.095,.3,'triangle');burst(4800,0,.052,.42,'highpass');burst(1700,.01,.12,.24);}
